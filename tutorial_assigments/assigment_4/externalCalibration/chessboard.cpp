...
int main(int argc, char **argv)
{
  ...
  for(int j=0;j<board_sz;j++)
    ...
    if(std::ifstream("cameraParameters.xml")){
      cv::FileStorage fs("cameraParameters.xml", cv::FileStorage::READ);
      if (!fs.isOpened())
      {
      ...
      cv::Mat rotation_vectors(3,1,cv::DataType<double>::type);
      cv::Mat translation_vectors(3,1,cv::DataType<double>::type);
  ...
  //    std::vector<cv::Mat> translation_vectors;
    ...
    //  ####################exer2
      ...
      for (i=0;i<n_boards;i++)
      {
        ...
        corner_count = FindAndDisplayChessboard(image.clone(),board_w,board_h,&corners);
        ...
        if (corner_count == board_w * board_h)
          {
            ...
            sucesses++;
            solvePnP(object_points.at(i),image_points.at(i),intrinsic,distCoeffs,rotation_vectors,translation_vectors);

            projectPoints(points,rotation_vectors,translation_vectors,intrinsic,distCoeffs,i_points);
    ...
    //  ####################exer2
      ...
      cv::String filenameToWrite = "cameraParameters.xml";
      cv::FileStorage fs(filenameToWrite, cv::FileStorage::WRITE);


      fs << "cameraMatrix" << intrinsic;                                      // Write cv::Mat
      fs << "distCoeffs" << distCoeffs;
